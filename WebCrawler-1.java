...
public class WebCrawler {
    ...
    // -----------------------------------------------------------------------
    // The only div we extract links from
    // -----------------------------------------------------------------------
    private static final String CONTENT_DIV_ID = "myportal-portlet-area";

    // -----------------------------------------------------------------------
    // Fetch a page, extract links ONLY from div#myportal-portlet-area
    // -----------------------------------------------------------------------
    private void processPage(String pageUrl) throws IOException {
        ...
        // Scope ALL link extraction to div#myportal-portlet-area only
        Element contentArea = doc.getElementById(CONTENT_DIV_ID);
        if (contentArea == null) {
            System.out.printf("  [WARN] div#%s not found on: %s — page skipped for link extraction%n",
                CONTENT_DIV_ID, pageUrl);
            if (thisRecord != null) thisRecord.status = "WARN: content div not found";
            return;
        }

        if (thisRecord != null) thisRecord.status = "OK";

        // Collect links only within the content div
        Set<String> rawLinks = new LinkedHashSet<>();
        for (Element el : contentArea.select("a[href]"))    rawLinks.add(el.attr("abs:href"));
        for (Element el : contentArea.select("link[href]")) rawLinks.add(el.attr("abs:href"));
        for (Element el : contentArea.select("img[src]"))   rawLinks.add(el.attr("abs:src"));
        for (Element el : contentArea.select("script[src]"))rawLinks.add(el.attr("abs:src"));
        for (Element el : contentArea.select("source[src]"))rawLinks.add(el.attr("abs:src"));
